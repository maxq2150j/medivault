...
const HospitalDashboard = () => {
    ...
    const handleSearch = async () => {
        try {
            ...
            console.log('[HospitalDashboard] Searching for:', searchQuery);
            const res = await api.get(`/hospital/search-patient?query=${encodeURIComponent(searchQuery)}`);
    ...
    const toggleDoctorActive = async (doctorId, currentStatus) => {
        try {
            ...
            showToast('danger', 'Error updating doctor status: ' + (err.response?.data?.message || err.message));
        }
    };
