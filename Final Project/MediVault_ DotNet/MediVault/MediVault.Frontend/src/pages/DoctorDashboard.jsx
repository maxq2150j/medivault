...
const DoctorDashboard = () => {
    ...
    useEffect(() => {
        ...
        console.log('[DoctorDashboard] doctorId from localStorage:', id);
        if (!id || id === 'undefined' || id === '0') {
            showToast('Invalid doctor ID. Please login again.', 'warning');
            return;
        }
        setDoctorId(id);
    }, []);
    ...
    const fetchDoctorProfile = async () => {
        try {
            ...
            showToast('Failed to fetch doctor profile: ' + (err.response?.data?.message || err.message), 'danger');
        }
    };
    ...
    const performSearch = async (query) => {
        ...
        try {
            ...
            const res = await api.get(`/doctor/search-patients?query=${encodeURIComponent(query)}`);
            ...
            showToast('Error searching patients: ' + (err.response?.data?.message || 'Unknown error'), 'danger');
        }
    };
    ...
    const requestAccess = async (patientId) => {
        try {
            ...
            setOtp('');
            showToast('OTP has been sent to patient email address.', 'success');
        } catch (err) {
            showToast(err.response?.data?.message || 'Failed to request access', 'danger');
        }
    };
    ...
    const verifyOTP = async () => {
        ...
        try {
            ...
            showToast('OTP verified! You can now record consultation.', 'success');
        } catch (err) {
            showToast(err.response?.data?.message || 'Invalid OTP', 'danger');
        }
    };
    ...
    const updateAppointmentStatus = async (appointmentId, status) => {
        try {
            if (status === 'Approved') {
                // Show payment modal for approval
            ...
            showToast(err.response?.data?.message || 'Failed to update appointment', 'danger');
        }
    };
    ...
    const requestPaymentFromPatient = async (e) => {
        ...
        try {
            ...
            showToast(err.response?.data?.message || 'Failed to request payment', 'danger');
        }
    };
    ...
    const generateConsultationReport = async (e) => {
        ...
        try {
            ...
            if (!doctorIdInt || !hospitalId) {
                showToast('Doctor or hospital not loaded. Please refresh and try again.', 'danger');
                return;
            }
            ...
            if (res?.data?.PdfUrl) {
                try {
                    ...
                    showToast('Consultation PDF generated and opened', 'success');
                } catch (err) {
                    console.error('Failed to open PDF:', err);
                    showToast('Consultation saved but failed to open PDF', 'warning');
                }
            } else {
                ...
                showToast('Consultation report saved and downloaded successfully', 'success');
            }
            ...
            showToast('Failed to generate report: ' + (err.response?.data?.message || err.message), 'danger');
        }
    };
                                            ...
                                            <div className="list-group-item" key={a.id}>
                                                <div className="d-flex w-100 justify-content-between align-items-center">
                                                    ...
                                                    <div className="d-flex align-items-center gap-2 flex-wrap">
                                                        ...
                                                        {a.status === 'Pending' && (
                                                            <>
                                                                <button className="btn btn-sm btn-success" onClick={() => updateAppointmentStatus(a.id, 'Approved')}>Approve</button>
